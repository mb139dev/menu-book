...
function showSuccessAlert(message) {
    ...
    alertElement.className = 'alert alert-success alert-dismissible fade show mb-2';
    ...
    filteredCategories.forEach(([categoryName, categoryProducts]) => {
        ...
        categoryProducts.forEach(product => {
            ...
            if (product.isBestSeller) {
                const badge = document.createElement('span');
                badge.className = 'badge best-seller-badge position-absolute';
...
function addToCart(product) {
    ...
    if (existingItem) {
        ...
        showSuccessAlert(`${product.name} quantity updated in cart!`);
    } else {
        cart.push({
            id: product.id,
            name: product.name,
        ...
        showSuccessAlert(`${product.name} added to cart successfully!`);
    }
